...
class LatinChars
{
    ...
    public IEnumerable<string> GetGlyphEnumerator()
    {
        foreach (string glyph in TestData.Groups.LatinLetters.Keys)
        {
            yield return glyph;
        }

        foreach (string glyph in TestData.Groups.NonLettersN1.Keys)
        {
            yield return glyph;
        }
